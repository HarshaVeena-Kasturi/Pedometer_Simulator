...
#define THRESHOLD   19660     // 2.5g (standard impact threshold)
...
TaskHandle_t Task1_Handle, Pedometer_Handle, Task3_Handle;
...
//   			Timer_Task1);
   ...
   xTaskCreate(Pedometer_Process, "Pedometer_Process", 128, NULL, 2, &Pedometer_Handle);
...
//    float gyro_y_dps;
//    int swing_detected=0;
//    while(1) {
//        // TODO: Calculate acceleration magnitude
//    	float aTotal = sqrtf((float)g_Ax * g_Ax + (float)g_Ay * g_Ay + (float)g_Az * g_Az) ;
...
//    	if((aTotal>=THRESHOLD)&&(swing_detected)&&(prev_magnitude<THRESHOLD)){
//    		step_count++;
//
...
//		//printf("NOTIFIED\r\n");
//	    xQueueReceive(alertQueue, &step_count,portMAX_DELAY);
//	    //printf("received\r\n");
//
//	 //   if(alert==1){
//	    	printf("Step count = %d\r\n",step_count);
